...
Notes:
  To run this markdown, need the rmarkdown package installed to execute the bash code blocks.
...
In this simple bash script, I:

1. Obtained the first line as from looking at the file content (gunzip -c <> | head ), I know it is the header.
2. I pass this line through conditional to check which separator it matches to

Limitations: This is assumes a few things

* It assumes that for tab deliminated files, spaces are not used in the column name (and vice versa)
...
Alternatively, it is often easier to obtain the first few rows, write it to file and look at it in vscode or vim and it will show the delimiter. We can to the head and tail to check, but it should all be the some

Limitation:
* It does assume that the entire file have the same separator, which is usually the case

It is always possible to parse through all the lines to make sure the number of columns are consistent, but usually in R if reading file with different columns counts, it will create an error (not always though)
...
1. (NR == FNR): technically not needed since just 1 file, though used for parsing more than 1 files, it help keeps track of where we are.
...
Similar to Question 4, will use awk to parse line by line, but with the extra step of first storing the requirement chromosome position
...
* This matches positions and chromosome exactly, as stated in the question, though can be modified similar to what is done in Question 4 to match a range. In which case, the first awk statement should just keep track of the min and max positions rather than all positions.
