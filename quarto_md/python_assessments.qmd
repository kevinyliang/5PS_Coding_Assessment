...
2. Number of genome-wide significant SNPs: ```{python} n_sig_var```
...
```{python}
#| label: locuszoom
#| fig-cap: Locuszoom around lead SNPs
fig,ax = plt.subplots()
sns.scatterplot(
  plot_data,
...
Number of palindromic ambiguous SNPs: ```{python} palin_dropped_snps```
